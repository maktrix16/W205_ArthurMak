...
MongoClient.connect(url, function(err, db) {
	...
	collection.aggregate(
		[
			// {
				// $match : {
				// 	$or: [
				// 		{"created_at" : {$gte:startDay1,$lt:endDay1}},
				// 		{"created_at" : {$gte:startDay2,$lt:endDay2}}
				// 	]
				// }
			// },
			{
			...
			console.log("\n"+docs[0]['_id']['screen_name']+" tweeted the most with "+docs[0]['num_tweets']+" tweets between 9am to 4pm CET on Feb 14 and Feb 15");
			// console.log(docs);
		}
	);
  ...
  setTimeout(function(){  //setting timeout so queries don't happen at the same time
	  ...
	  collection.find(
		  {
				$or: [
					{"created_at" : {$gte:startDay1,$lte:endDay1}},
					{"created_at" : {$gte:startDay2,$lte:endDay2}}
				]
		  },
		  {hashtags:1}
	  ).toArray(function(err,docs){
	  	hashtags_count = count_hashtags(docs);
	  	console.log("\nThese are the top 10 most used hashtags between 9am and 4pm CET on Feb 14 and Feb 15:");
  ...
  setTimeout(function(){  //setting timeout so queries don't happen at the same time
		...
		console.log("\nHourly tweets in chronological order:");
		(function loop() {
			collection.aggregate([
				{
